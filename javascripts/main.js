...
$(document).on("change", '#range', function() {
  // $(this).val = var slidervalue;
  var slidervalue = $(this).val();
  console.log(slidervalue);
  //call set filter 5 stars
  //need to pass all movies from firebase
  //
  dataControl.getUsersMovies()
    .then(function(allMovies){
        domControl.loadProfileHbs(dataControl.setFilter5stars(allMovies, slidervalue));
    });
...
//we will need to grab the value of the slider and pass it to the datacontrol set filter 5 star function
  ...
  $(document).on('click', '#filterAll', function() {
     ...
     $('#filterToWatch').removeClass("active");
     $('#filterWatched').removeClass("active");
     $('#filterRated5').removeClass("active");
  });
